...
Online rooms use PeerJS and its public signaling service to connect browsers over WebRTC. This is an external dependency; GitHub Pages cannot run a signaling service. Connections explicitly use Google and Cloudflare STUN services instead of the defunct TURN addresses bundled in PeerJS 1.5.5. A disconnected signaling socket reconnects automatically without ending existing data channels. Joining has longer negotiation windows, one automatic retry, and separate errors for a host that did not answer, incompatible versions, and a host that answered but could not connect. No working TURN account is configured yet. That is a fallback limitation, not proof of the cause of a failed join. Networks with compatible NAT behavior can connect directly. An optional relay setup is described below. The game now loads provider-issued TURN credentials before creating either a host or guest connection, including UDP and TLS/TCP endpoints. If a configured provider cannot be reached or returns no usable TURN credentials, the game reports that failure instead of silently reverting to direct-only connections. The old pre-hot-join build used the same discontinued PeerJS TURN fallback, so that fallback alone does not establish the cause of a particular failed invite. See the [PeerJS discontinuation notice](https://github.com/orgs/peers/discussions/1172). There is no voice/video capture, user account, analytics, or gameplay database.
