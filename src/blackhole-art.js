export function drawBlackhole(r, f, time) {
  ...
  for (let n = 0; n < 9; n++) {
    ...
    for (let i = 0; i < 22; i++) {
      const d = f.radius * (1 - i / 23) * grow,
        a = start + i * 0.14;
      points.push([f.x + Math.cos(a) * d, f.y + Math.sin(a) * d * 0.88]);
    }
  ...
  for (let n = 0; n < 32; n++) {
    ...
    r.line(
      [
        [f.x + Math.cos(a) * radius, f.y + Math.sin(a) * radius * 0.88],
        [
          f.x + Math.cos(a - 0.08) * radius,
          f.y + Math.sin(a - 0.08) * radius * 0.88,
        ],
      ],
      n % 2 ? "#c4a8ff" : "#ffceec",
      2,
  ...
  c.translate(f.x, f.y);
  c.rotate(-0.35);
  c.strokeStyle = "#f3ddffbb";
  c.lineWidth = 4;
  c.beginPath();
  c.ellipse(0, 0, size * 1.8, size * 0.35, 0, 0, Math.PI * 2);
  c.stroke();
  c.restore();
}
