import { blastFurnace, furnaceHits } from './furnace-parts.js';
...
function addWreck(world, f, s, kind = "platform") {
  ...
  world.wreckage.push({
    ...
    ...(s.kind === "car" ? {carStage:s.carStage??0,carPaint:s.carPaint??0,carCoat:s.carCoat??0} : {}),
    sourceChunk: !!s.chunk,
    shape: s.shape,
...
export function updateBlackhole(world, f, dt) {
  ...
  for (const key of ["water", "gas", "spills"]) world[key] = (world[key] || []).filter(q => {
    ...
    collectMatter(world, f, { ...q, color:key === "water" ? "#69c5ee" : key === "gas" ? "#b0c99f" :
      q.kind === "molten" ? "#ff842b" : q.kind === "oil" ? "#827757" : q.kind === "glue" ? "#d3dba0" : "#6e497e" }, "debris");
    return false;
  });
