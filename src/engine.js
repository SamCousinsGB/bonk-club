  ...
  step(dt, inputs = {}) {
    ...
    if (!active) {
      ...
      if (this.grenadeTimer <= 0) {
        const type = chooseWeapon(this.random, new Set(this.drops.map(d => d.type)), this.weaponPool);
        if (type && WEAPONS[type].kind === "grenade") this.spawnWeapon(type);
