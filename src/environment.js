...
import { HAZARD_LABELS } from "./hazards.js";
...
export function drawHazards(c, hazards, time) {
  for (const h of hazards || []) {
    ...
    const left = h.x - h.w / 2,
      top = h.y - h.h;
    if (h.warning <= 0 && !["rockfall", "cargo"].includes(h.type)) {
      c.beginPath();
      c.rect(left, top, h.w, h.h);
      c.clip();
    }
    if (h.warning > 0) {
      c.fillStyle = "#ffbe4920";
      c.fillRect(left, top, h.w, h.h);
      c.strokeStyle = "#ffd078";
      c.lineWidth = 3;
      c.setLineDash([10, 8]);
      c.strokeRect(left, top, h.w, h.h);
      c.setLineDash([]);
      for (let x = left; x < left + h.w; x += 22)
        line(
          c,
          [
            [x, h.y - 4],
            [Math.min(x + 12, left + h.w), h.y - 16],
          ],
          "#ffd078",
          3,
        );
      c.font = "700 20px 'DM Sans',sans-serif";
      c.textAlign = "center";
      c.fillStyle = "#ffe0a1";
      c.fillText(HAZARD_LABELS[h.type], h.x, top - 16);
      circle(c, h.x, top + 28, 12, "#ffd078");
      c.fillStyle = "#3f342c";
      c.font = "bold 20px sans-serif";
      c.fillText("!", h.x, top + 35);
    } else if (h.type === "rockfall" || h.type === "cargo") {
      c.globalAlpha = h.done ? Math.max(0, (h.duration - h.age) / 0.45) : 1;
      if (h.type === "cargo") {
        c.fillStyle = "#ad865a";
        c.fillRect(h.x - 26, h.bodyY - 26, 52, 52);
        line(
          c,
          [
            [h.x - 22, h.bodyY - 22],
    ...
    } else if (h.type === "lightning" || h.type === "electric") {
      const on = h.type === "lightning" || h.age % 1 < 0.45;
      c.fillStyle = on ? "#8ae9ff22" : "#8ae9ff0a";
      c.fillRect(left, top, h.w, h.h);
      if (on) {
        const pts = [];
        for (let i = 0; i < 8; i++)
          pts.push([
            h.x + Math.sin(i * 9 + h.age * 20) * h.w * 0.32,
            top + (i * h.h) / 7,
          ]);
        line(c, pts, "#6cdfff55", 17);
        line(c, pts, "#d9fdff", 4);
      }
      line(
        c,
      ...
      for (let i = 0; i < 15; i++) {
        const x = left + ((((i * 29 + h.age * 160 * h.dir) % h.w) + h.w) % h.w),
          y = top + noise(i) * h.h;
        line(
          c,
          [
      ...
      c.fillStyle = "#aecb591c";
      c.fillRect(left, top, h.w, h.h);
      for (let i = 0; i < 24; i++) {
        const x = left + noise(i) * h.w,
          y = top + noise(i * 9.37 + 8) * h.h;
        circle(
          c,
          x + Math.sin(h.age + i) * 9,
          y,
          20 + noise(i + 3) * 20,
          "#aecb5930",
        );
      ...
      if (on)
        for (let i = 0; i < 20; i++) {
          const f = (h.age * 1.8 + i / 20) % 1,
            x = h.x + Math.sin(i * 7) * h.w * 0.35 * f;
          circle(
            c,
            x,
            h.y - f * h.h,
            8 + (1 - f) * 14,
            lava ? "#fca05a88" : "#a6dbda44",
          );
        }
    }
    c.restore();
