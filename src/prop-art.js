...
function sourceArtwork(r, b) {
  const [, , , , w, h] = b.sourceArt, key = `${b.kind}:${w}:${h}:${b.carStage}:${b.carPaint}:${b.carCoat}`;
  ...
  r.ctx = c;
  try { r.tableArt({kind:b.kind,carStage:b.carStage,carPaint:b.carPaint,carCoat:b.carCoat, x:0, y:0, w, h, hp:100, maxHp:100}); }
  finally { r.ctx = main; }
...
function shardArtwork(r, b) {
  const key=JSON.stringify([b.kind,b.material,b.sourceArt,b.shape,b.carStage,b.carPaint,b.carCoat]),cache=(r.shardArt ||= new Map());
  if(cache.has(key))return cache.get(key);
