...
export function captureFighter(p, f) {
  ...
  p.captureAge = 0;
  p.knockdown = 1;
  p.block = false; p.swing = 0; p.freeze = 0; p.burn = 0;
  p.prone = false; p.ground = false; p.support = null; p.jumpBuffer = 0;
  for (const q of new Set(p.strands.flatMap(s => s.points))) seedOrbit(q, f, .45);
}
...
export function orbitBody(strands, center, solids, dt, drift = .45) {
  ...
  for (const q of points) {
    ...
    q.px -= nx*tide*dt*dt; q.py -= ny*tide*dt*dt;
    orbitPoint(q, center, dt, drift, center.life > 1.1 ? 130 : 0);
  }
  for (let n = 0; n < 3; n++) {
