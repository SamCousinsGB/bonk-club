...
export function updateTrain(world,h,dt) {
  ...
  const sweep={x:Math.min(previous,h.bodyX)-h.w/2,y:h.y-h.h,w:h.w+Math.abs(h.bodyX-previous),h:h.h};
  for(const p of world.players)if(p.alive&&overlap(playerBox(p),sweep)){
    p.vx=h.dir*1450;p.vy=-380;
  ...
  for(const rag of world.ragdolls){
    ...
    for(const q of rag.points)if(overlap({x:q.x-4,y:q.y-4,w:8,h:8},sweep)){
      q.px=q.x-h.dir*1450*dt;q.py=q.y+380*dt;
    }
  }
  for(const b of world.cover)if(b.hp>0&&overlap(b,sweep)){b.vx=h.dir*1200;b.vy=-300;world.damageCover(b,1000);}
  for(const b of world.chunks)if(b.hp>0&&overlap(b,sweep)){b.vx=h.dir*1200;b.vy=-300;b.spin=h.dir*8;}
  for(const d of world.drops)if(overlap({x:d.x-8,y:d.y-8,w:16,h:16},sweep)){d.vx=h.dir*1200;d.vy=-300;}
