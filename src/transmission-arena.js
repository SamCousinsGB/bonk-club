...
export const WIRE_LEFT = 820, WIRE_RIGHT = 1740, WIRE_SEGMENTS = 40;
...
for (const [circuit,y] of [440,860].entries()) {
  for (let i=0;i<WIRE_SEGMENTS;i++) {
    const u=i/(WIRE_SEGMENTS-1),w=(WIRE_RIGHT-WIRE_LEFT)/WIRE_SEGMENTS;
    // Each rise is at most three units, within the shared movement solver's
    // landing tolerance. The visible cable and its solid top are the same.
    platforms.push(deck(WIRE_LEFT+i*w,y+Math.round(28*4*u*(1-u)),w,6,{material:"cable",circuit}));
  }
}
...
export const TRANSMISSION_ARENA = dressArena({
  ...
  traps:[440,860].map((y,circuit)=>({type:"powerline",circuit,x:1280,y:y+28,
    w:WIRE_RIGHT-WIRE_LEFT,h:70,dir:circuit?1:-1})),
});
