...
const STEP = 1 / 120, LIMIT = 128, STALE = 250;
...
export class WeaponPrediction {
  ...
  reset() { this.pending = new Map(); this.effects = []; this.played = new Map(); }
  ...
  position(r, now) {
    const b={...r.entity}, dt=Math.max(0,Math.min(STALE,now-r.at))/1000;
  ...
  receive(state,id,now) {
    ...
    for(const [id,r] of this.pending) {
      ...
      } else if(r.confirmed || ack>=r.seq || !p?.alive || p.knockdown || p.freeze || p.strands || now-r.at>STALE) {
  ...
  sample(state,now) {
    if(!this.pending.size)return state;
    const out={...state,projectiles:[...state.projectiles],drops:[...state.drops],fields:[...state.fields]};
    const solids=[...state.platforms,...state.cover,...state.chunks].filter(s=>s.hp!==0);
    for(const [id,r] of this.pending) {
      if(now-r.at>STALE){this.pending.delete(id);continue;}
      ...
      if(index>=0)list[index]=b;else if(!r.confirmed)list.push(b);
    }
    return out;
  }
}
