...
test('walking in either direction stays silent, including repeated frames', () => {
  for (const dir of [1,-1]) {
    const steps=new Landings(),p=player(),contacts=[];
    for(let i=0;i<=120;i++){
...
test('idle, blocked, lift travel, prone, frozen and recovering fighters have no steps', () => {
  for(const patch of [{}, {prone:true}, {ground:false}, {freeze:1}, {knockdown:1}, {alive:false}, {morphTime:1}]){
    const steps=new Landings(),contacts=[];
...
test('landing makes one heavier contact and no airborne footsteps', () => {
  const steps=new Landings();steps.update(state(0,player()));
...
test('hot join, stalls, teleports, round reset and occupant replacement cannot replay old contacts', () => {
  const steps=new Landings();assert.deepEqual(steps.update(state(3,player({walk:70}))),[]);
...
test('four fighters keep independent contact timing; absent players leave no tracker state', () => {
  const steps=new Landings(),players=[0,1,2,3].map(id=>player({id,ground:false,vy:300}));
  steps.update({...state(0,players[0]),players});
  const contacts=steps.update({...state(.21,players[0]),players:players.map(p=>({...p,ground:true,vy:0}))});
  assert.equal(contacts.length,4);
  steps.update(state(.23,player()));assert.equal(steps.players.size,1);
});
test('real host and interpolated guest walking stay silent; jumping keeps one landing', () => {
  ...
  const host=new Landings(),guest=new Landings(),wire=new RenderSnapshots();
  ...
  assert.equal(hostCount,0);assert.equal(guestCount,0);
  for(let i=0;i<60;i++){
    w.step(STEP,{0:{right:false,left:false,jump:false,duck:false,aim:0}});
    assert.deepEqual(host.update(w.snapshot()),[]);
  }
  previous=wire.make(w.snapshot());guest.update(previous);
  for(let i=0;i<240;i++){
    w.step(STEP,{0:{jump:i===0,aim:0}});
    hostCount+=host.update(w.snapshot()).length;
    if(i%4===3){
      const next=wire.make(w.snapshot());
      guestCount+=guest.update(interpolateStates(previous,next,.5)).length;
  ...
  assert.equal(hostCount,1);assert.equal(guestCount,1);
});
