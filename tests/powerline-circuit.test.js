...
test("eight larger water tanks occupy every pylon arm and the four centre weapon ledges are absent",()=>{
  ...
  for(const y of TOWER_LEVELS)for(const [left,right] of [[50,480],[820,990],[1570,1740],[2080,2510]]) {
    ...
    assert.ok(j,`jug on arm ${left},${y}`);assert.equal(j.waterLeft,j.waterCapacity);assert.ok(j.waterLeft>=450&&j.waterLeft<=1200);assert.ok(j.mass>0);
  }
  ...
  jugs[0].waterLeft=0;w.startRound();assert.equal(w.cover.filter(b=>b.kind==="waterTank"&&b.waterLeft===b.waterCapacity).length,8);
});
