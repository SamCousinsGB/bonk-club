...
test("every airborne weapon retains its projectile beyond its old timeout while still in play",()=>{
  for(const [type,definition] of Object.entries(WEAPONS)) {
    ...
    for(let t=0;t<12;t+=STEP){if(b.kind==="duck")b.vy=0;w.updateProjectiles(STEP);}
    assert.ok(w.projectiles.includes(b),type);assert.ok(b.age>11.9,type);assert.ok(b.life>0,type);
    assert.ok(b.x<600,type);assert.equal(w.fields.length,0,type);
...
test("grenade fuses still detonate while far outside the previous boundary",()=>{
  ...
  b.life=.01;
  for(let t=0;t<.05;t+=STEP)w.updateProjectiles(STEP);
