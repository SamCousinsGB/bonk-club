...
for(const direction of [-1,1])test(`a fighter crosses the sagging safe wire in both directions (${direction})`,()=>{
...
test("live contact shocks only the actual wire, with bounded repeat hits and electrical deaths",()=>{
  ...
  h.age=7;Object.assign(p,{x:1280,y:438,hp:100});Object.assign(q,{x:1280,y:340,hp:100});
...
test("prediction never applies wire damage and countdown/results do not advance circuits",()=>{
  ...
  h.age=7;Object.assign(p,{x:1280,y:438});advance(w,.1);assert.equal(p.hp,100);
  for(const phase of ["countdown","result"]){w.phase=phase;const age=h.age;advance(w,20);assert.equal(h.age,age);}
});
test("explosions sever a circuit permanently, retain solid surviving wire and reset next round",()=>{
  const w=arena(),h=w.hazards[0];advance(w,7);
  carveExplosion(w,{x:1110,y:465,radius:65});updateHazards(w,STEP);
...
test("hot join and interpolation retain circuit timing and destroyed wire geometry; malformed fields reject",()=>{
  const w=arena();advance(w,7);carveExplosion(w,{x:1110,y:465,radius:65});updateHazards(w,STEP);
  ...
  for(const patch of [{circuit:2},{circuit:"0"},{w:1001},{duration:7.1},{active:"yes"}]) {
    const s=structuredClone(state);Object.assign(s.hazards[0],patch);assert.equal(validSnapshot(s),false);
  }
  const s=structuredClone(state);s.platforms.find(p=>p.material==="cable").circuit=9;assert.equal(validSnapshot(s),false);
});
